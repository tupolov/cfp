

/*ChestRig1

class CfgVehicleClasses
{
	class CFP_ChestRig
	{
		displayName = "Chest Rig";
	};
};
*/

	class SP_ChestRig1_Black: V_PlateCarrier1_rgr
{
    picture = "\x\cfp\addons\vests\data\ui\icon_SP_ChestRig1_Black.paa";
		scope = 2;
		displayName = "Chest Rig 1 (Black)";
		// picture = "\x\cfp\addons\vests\ChestRig1\ui\Black.jpg";
		model = "\x\cfp\addons\models\ChestRig1";
		hiddenSelectionsTextures[] = {"\x\cfp\addons\vests\ChestRig1\Black.paa","\x\cfp\addons\vests\ChestRig1\Black2.paa"};
		hiddenSelections[] = {"Camo1","Camo2"};
		author = "CFP";



		class ItemInfo: VestItem
		{
			uniformModel = "\x\cfp\addons\models\ChestRig1";
			containerClass = "Supply80";
			hiddenSelections[] = {"Camo1","Camo2"};
			mass = 100;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					hitpointName = "HitChest";
					armor = 14;
					passThrough = 0.2;
				};
				class Diaphragm
				{
					hitpointName = "HitDiaphragm";
					armor = 14;
					passThrough = 0.2;
				};
				class Abdomen
				{
					hitpointName = "HitAbdomen";
					armor = 14;
					passThrough = 0.2;
				};
				class Body
				{
					hitpointName = "HitBody";
					passThrough = 0.2;
				};
			};

		};
	};

	class SP_ChestRig1_Green: V_PlateCarrier1_rgr
{
    picture = "\x\cfp\addons\vests\data\ui\icon_SP_ChestRig1_Green.paa";
		scope = 2;
		displayName = "Chest Rig 1 (Green)";
		// picture = "\x\cfp\addons\vests\ChestRig1\ui\Green.jpg";
		model = "\x\cfp\addons\models\ChestRig1";
		hiddenSelectionsTextures[] = {"\x\cfp\addons\vests\ChestRig1\Green.paa","\x\cfp\addons\vests\ChestRig1\Green2.paa"};
		hiddenSelections[] = {"Camo1","Camo2"};
		author = "CFP";



		class ItemInfo: VestItem
		{
			uniformModel = "\x\cfp\addons\models\ChestRig1";
			containerClass = "Supply80";
			hiddenSelections[] = {"Camo1","Camo2"};
			mass = 100;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					hitpointName = "HitChest";
					armor = 14;
					passThrough = 0.2;
				};
				class Diaphragm
				{
					hitpointName = "HitDiaphragm";
					armor = 14;
					passThrough = 0.2;
				};
				class Abdomen
				{
					hitpointName = "HitAbdomen";
					armor = 14;
					passThrough = 0.2;
				};
				class Body
				{
					hitpointName = "HitBody";
					passThrough = 0.2;
				};
			};

		};
	};

	class SP_ChestRig1_Tan: V_PlateCarrier1_rgr
{
    picture = "\x\cfp\addons\vests\data\ui\icon_SP_ChestRig1_Tan.paa";
		scope = 2;
		displayName = "Chest Rig 1 (Tan)";
		// picture = "\x\cfp\addons\vests\ChestRig1\ui\Tan.jpg";
		model = "\x\cfp\addons\models\ChestRig1";
		hiddenSelectionsTextures[] = {"\x\cfp\addons\vests\ChestRig1\Tan.paa","\x\cfp\addons\vests\ChestRig1\Tan2.paa"};
		hiddenSelections[] = {"Camo1","Camo2"};
		author = "CFP";



		class ItemInfo: VestItem
		{
			uniformModel = "\x\cfp\addons\models\ChestRig1";
			containerClass = "Supply80";
			hiddenSelections[] = {"Camo1","Camo2"};
			mass = 100;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					hitpointName = "HitChest";
					armor = 14;
					passThrough = 0.2;
				};
				class Diaphragm
				{
					hitpointName = "HitDiaphragm";
					armor = 14;
					passThrough = 0.2;
				};
				class Abdomen
				{
					hitpointName = "HitAbdomen";
					armor = 14;
					passThrough = 0.2;
				};
				class Body
				{
					hitpointName = "HitBody";
					passThrough = 0.2;
				};
			};

		};
	};

	class SP_ChestRig1_Grey: V_PlateCarrier1_rgr
{
    picture = "\x\cfp\addons\vests\data\ui\icon_SP_ChestRig1_Grey.paa";
		scope = 2;
		displayName = "Chest Rig 1 (Grey)";
		// picture = "\x\cfp\addons\vests\ChestRig1\ui\Grey.jpg";
		model = "\x\cfp\addons\models\ChestRig1";
		hiddenSelectionsTextures[] = {"\x\cfp\addons\vests\ChestRig1\Grey.paa","\x\cfp\addons\vests\ChestRig1\Grey2.paa"};
		hiddenSelections[] = {"Camo1","Camo2"};
		author = "CFP";



		class ItemInfo: VestItem
		{
			uniformModel = "\x\cfp\addons\models\ChestRig1";
			containerClass = "Supply80";
			hiddenSelections[] = {"Camo1","Camo2"};
			mass = 100;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					hitpointName = "HitChest";
					armor = 14;
					passThrough = 0.2;
				};
				class Diaphragm
				{
					hitpointName = "HitDiaphragm";
					armor = 14;
					passThrough = 0.2;
				};
				class Abdomen
				{
					hitpointName = "HitAbdomen";
					armor = 14;
					passThrough = 0.2;
				};
				class Body
				{
					hitpointName = "HitBody";
					passThrough = 0.2;
				};
			};
		};
	};
