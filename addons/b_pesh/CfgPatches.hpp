class DefaultEventhandlers;
class CfgPatches
{
	class ADDON
	{
		units[] = {
            "cfp_b_pesh_leader",
            "cfp_b_pesh_grenadier",
            "cfp_b_pesh_rifleman",
            "cfp_b_pesh_machinegunner",
            "cfp_b_pesh_sniper",
            "cfp_b_pesh_at",
            "cfp_b_pesh_medic",
            "cfp_b_pesh_crewman",
            "cfp_b_pesh_rifleman_dshkm",
            "cfp_b_pesh_rifleman_kord",
            "cfp_b_pesh_rifleman_podnos",
            "cfp_b_pesh_rifleman_ags30",
            "cfp_b_pesh_rifleman_metis",
            "cfp_b_pesh_rifleman_spg9",
            "cfp_b_pesh_asst_gunner_dshkm",
            "cfp_b_pesh_asst_gunner_kord",
            "cfp_b_pesh_asst_gunner_podnos",
            "cfp_b_pesh_asst_gunner_ags30",
            "cfp_b_pesh_asst_gunner_metis",
            "cfp_b_pesh_asst_gunner_spg9",
            "cfp_b_pesh_offroad",
            "cfp_b_pesh_offroad_M2",
            "CFP_B_PESH_Hilux_Unarmed_01",
            "CFP_B_PESH_Hilux_DSHKM_01",
            "CFP_B_PESH_Hilux_SPG9_01",
            "CFP_B_PESH_Hilux_Podnos_01",
            "CFP_B_PESH_Hilux_ZU_23_01",
            "CFP_B_PESH_Hilux_MLRS_01",
            "CFP_B_PESH_HMMWV_M2_GPK",
            "CFP_B_PESH_HMMWV_Mk19_GPK",
            "cfp_b_pesh_HMMWV_M2",
            "cfp_b_pesh_Ural",
            "cfp_b_pesh_Ural_ZU23",
		"cfp_b_pesh_Ural_Refuel_01",
		"cfp_b_pesh_Ural_Repair_01",
		"cfp_b_pesh_Ural_Ammo_01",
            "cfp_b_pesh_BRDM",
            "cfp_b_pesh_BTR60",
            "cfp_b_pesh_BMP1",
            "cfp_b_pesh_BMP2",
            "cfp_b_pesh_ZSU",
            "cfp_b_pesh_2b14",
            "cfp_b_pesh_Metis",
            "cfp_b_pesh_ags30",
            "cfp_b_pesh_dshkm",
            "cfp_b_pesh_dshkm_minitripod",
            "cfp_b_pesh_zu23",
		"ISC_Flag_Kurdistan"
		};
		weapons[] = {
			"PESH_Vest",
			"PESH_Beret",
			"PESH_Beret_CTG",
			"PESH_uniform"
		};
		requiredAddons[] = {
            "cfp_main",
			"A3_Dubbing_Radio_F",
			"A3_Characters_F",
			"A3_Characters_F_Common",
			"A3_Characters_F_Civil",
			"A3_Characters_F_Gamma",
			"A3_Characters_F_Bootcamp",
			"A3_Characters_F_BLUFOR",
            "A3_Soft_F_Offroad_01",
            "CUP_WheeledVehicles_HMMWV",
            "CUP_WheeledVehicles_Ural",
            "CUP_WheeledVehicles_BRDM2",
            "CUP_WheeledVehicles_BTR60",
            "CUP_TrackedVehicles_BMP",
            "CUP_TrackedVehicles_ZSU23",
            "CUP_Weapons_StaticWeapons",
            "CUP_Creatures_StaticWeapons"
        };
        author = MODULE_AUTHOR;
        authors[] = {"AccuracyThruVolume"};
        requiredVersion = REQUIRED_VERSION;
        VERSION_CONFIG;
	};
};
